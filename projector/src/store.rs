...
pub async fn record_credit_deposited(event: CreditDeposited) -> Result<()> {
...
/// Records a reservation of credit by adding a credit reserved transaction to the end of the
/// ledger and recording the newly adjusted balance
pub async fn record_credit_reserved(event: CreditReserved) -> Result<()> {
    debug!(
        "Recording credit reservation (intermutualcredit) in account {}",
    ...
    let new_ledger = get(&ctx, &kv, &ledger_key).await.map(|ledger_raw| {
        serde_json::from_str::<AccountLedger>(&ledger_raw).map(|mut ledger| {
            ...
            ledger
                .holds
                .insert(event.credit_transfer_id, event.amount as u32);
            ledger.ledger_lines.push(LedgerLine {
                amount: event.amount as u32,
                tx_type: TransactionType::CreditReserve,
                effective_balance: last_balance - event.amount as u32,
            });
            ledger
        })
...
// Releases previously reserved credit by adding a credit released transaction to the end
/// of the ledger and recording the updated balance
pub async fn record_credit_released(event: CreditReleased) -> Result<()> {
    debug!(
        "Recording credit release (intermutualcredit) in account {}",
        event.account_number
    );
    let account_number = event.account_number.to_string();
    ...
    let new_ledger = get(&ctx, &kv, &ledger_key).await.map(|ledger_raw| {
        serde_json::from_str::<AccountLedger>(&ledger_raw).map(|mut ledger| {
            let last_balance = ledger.ledger_lines.last().unwrap().effective_balance;
            let orig_hold = ledger.holds.remove(&event.credit_transfer_id);
            ledger.ledger_lines.push(LedgerLine {
                amount: orig_hold.unwrap_or_default(),
                tx_type: TransactionType::CreditRelease,
...
/// ledger and recording the new balance
pub async fn record_credit_withdrawn(event: CreditWithdrawn) -> Result<()> {
